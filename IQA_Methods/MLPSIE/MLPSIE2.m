function [output] = MLPSIE2(imgTC,data)
...
if size(imgTC,4) == 1
    if isstring(imgTC) || ischar(imgTC) || size(imgTC,1)==1 || size(imgTC,3)==3
        for numberLayer =   1 : size(data.ConstantsNetVgg19.Lengths,2)
            ...
            actValues= activations(data.ConstantsNetVgg19.net, imgDist_TC, data.ConstantsNetVgg19.Layers{numberLayer},'OutputAs','rows');

            matrixIamgesFeatures = [actValues];
        ...
        for numberLayer =   1 : size(data.ConstantsNetResnet18.Lengths,2)
            ...
            actValues= activations(data.ConstantsNetResnet18.net, imgDist_TC, data.ConstantsNetResnet18.Layers{numberLayer},'OutputAs','rows');
        ...
        for i =1:size(imgTC,1)
            ...
            for numberLayer =   1 : size(data.ConstantsNetVgg19.Lengths,2)
                ...
                actValues= activations(data.ConstantsNetVgg19.net, imgDist_TC{i}, data.ConstantsNetVgg19.Layers{numberLayer},'OutputAs','rows');
                matrixIamgesFeatures = [actValues];
        ...
        for i =1:size(imgTC,1)
            ...
            for numberLayer =   1 : size(data.ConstantsNetResnet18.Lengths,2)
                ...
                actValues= activations(data.ConstantsNetResnet18.net, imgDist_TC{i}, data.ConstantsNetResnet18.Layers{numberLayer},'OutputAs','rows');
                matrixIamgesFeatures = [actValues ];
